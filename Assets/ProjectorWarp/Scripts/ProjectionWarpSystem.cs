...
namespace MultiProjectorWarpSystem
{
    ...
    public class ProjectionWarpSystem : MonoBehaviour
    {
        ...
        {   
            ...
            switch (arrangement)
            {
                case CameraArragement.HORIZONTAL_ORTHOGRAPHIC:
                    ...
                    for (int i = 0; i < sourceCameras.Count; i++)
                    {
                        offsetX = startX + (i * viewportWidth * orthographicSizeScale) - (i * overlap.x * orthographicSizeScale);
                ...
                case CameraArragement.VERTICAL_ORTHOGRAPHIC:
                    ...
                    for (int i = 0; i < sourceCameras.Count; i++)
                    {
                        offsetY = startY + (i * viewportHeight * orthographicSizeScale) - (i * overlap.y * orthographicSizeScale);
                        sourceCameras[i].transform.localPosition = new Vector3(
                            0,
                            offsetY,
                            0);
        ...
        {        
            ...
            for (int i = 0; i < projectorCount; i++)
            {
                ...
                switch(arrangement){
                    case CameraArragement.VERTICAL_ORTHOGRAPHIC:
                    case CameraArragement.VERTICAL_PERSPECTIVE:
                        projectionCamera.transform.localPosition = new Vector3(0f, (float)i * (projectionMesh.height + projectionCameraSpace), 0f);
                        break;
                    case CameraArragement.HORIZONTAL_ORTHOGRAPHIC:
                    case CameraArragement.HORIZONTAL_PERSPECTIVE:
                    default:
        ...
        public void SaveCalibration(string path)
        {
            ...
            json += "\"OrthographicSizeScale\":" + orthographicSizeScale + ",";
            ...
            var sr = File.CreateText(Application.streamingAssetsPath + "/" + configFileName);
            sr.WriteLine(json);
            sr.Close();
